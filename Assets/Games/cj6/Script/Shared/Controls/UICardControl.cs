...
//		public void SetCardData(byte cardata, byte count)
//		{
//			//清空
//			if (count == 0)
//			{
...
//			}
			...
			for (int i = 0; i < _cardcount; i++)
			{
				...
				float zValue = ((float)i) / 100 + 1;
//				Vector2 size = target.GetComponentInChildren<UISlicedSprite>().localSize;
//				float xValue = size.x / obj.GetComponent<UIWidget>().localSize.x;
//				float yValue = size.y / obj.GetComponent<UIWidget>().localSize.y;

				obj.transform.localScale = new Vector3( 1, 1, zValue);

				int nRow = (int)(i / RowMaxCards);
				int nCol = (int)(i % RowMaxCards);
				Vector3 point = target.transform.localPosition;
				float card_width = obj.transform.FindChild("Background").GetComponent<UIWidget>().localSize.x;

				obj.transform.localPosition = point - new Vector3(-1*i*card_width/2 + card_width/2,0,0);
