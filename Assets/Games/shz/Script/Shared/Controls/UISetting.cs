...
//		static GameObject o_btn_speak_set = null;
		...
		void Start(){
			...
			mVol=UIManager.Instance.o_game.GetComponent<AudioSource>().volume;
...
//			GameObject.Find("Panel").GetComponent<AudioSource>().volume = mVol;
			UIManager.Instance.o_game.GetComponent<AudioSource>().volume = mVol;
			//喇叭按钮的显示
		...
		void OnMusicCloseIvk()
		{
			...
			{
				PlayerPrefs.SetString("game_music_switch", "on");
				UIManager.Instance.o_game.GetComponent<AudioSource>().Play();
				o_set_btn_music.GetComponent<UISprite>().spriteName = "btn_audio_on";
				o_set_music.gameObject.GetComponent<UISlider>().sliderValue = 0.5f;
				PlayerPrefs.SetFloat("game_music", 0.5f);
				UIManager.Instance.o_game.GetComponent<AudioSource>().volume = 0.5f;
			}
			else
			{
				PlayerPrefs.SetString("game_music_switch", "off");
				UIManager.Instance.o_game.GetComponent<AudioSource>().Pause();
				o_set_btn_music.GetComponent<UISprite>().spriteName = "btn_audio_off";
				
				o_set_music.gameObject.GetComponent<UISlider>().sliderValue = 0;
				PlayerPrefs.SetFloat("game_music", 0);
				UIManager.Instance.o_game.GetComponent<AudioSource>().volume = 0;
			}
		}
...
//			UIGame.btn_chat.GetComponent<UIButton>().isEnabled=true;
		...
		public void Show(bool bshow)
		{
			...
			if (bshow == true)
			{
				...
				UIManager.Instance.o_game.GetComponent<AudioSource>().volume=o_set_music.gameObject.GetComponent<UISlider>().value;
...
//					UIGame.btn_chat_disabled.SetActive(false);
				...
				{
					o_set_btn_effect.GetComponentInChildren<UISprite>().spriteName = "btn_audio_on";
				}
