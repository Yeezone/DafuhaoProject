Shader "DistortShader" 
{
	Properties 
	{
		...
		_DispMap ("Distort Map (RG)", 2D) = "white" {}
		_DispScrollSpeedX  ("Map Scroll Speed X", Float) = 0
		_DispScrollSpeedY  ("Map Scroll Speed Y", Float) = 0
		_StrengthX  ("Dis Strength X(Base on Distort Map)", Float) = 1
		_StrengthY  ("Dis Strength Y(Base on Distort Map)", Float) = -1
	}
		...
		SubShader 
		{
			...
			Pass
			{
				...
				half4 frag( v2f i ) : COLOR
				{
					...
					//use mask's red channel to determine visbility.
