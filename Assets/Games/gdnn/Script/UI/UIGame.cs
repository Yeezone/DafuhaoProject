...
namespace com.QH.QPGame.GDNN
{
	...
	public class UIGame : MonoBehaviour
	{
		...
		//配置数据
		private string systemBankerName;
		//系统坐庄名字
		...
		//时间控制
		public float sendCardTypeTime = 0.9f;
		//发每组牌的时间间隔
		public float showCardTypeTime = 2.0f;
		//开每组牌的时间间隔
		public float oneCardTime = 0.1f;
		//发牌时两张牌时间的间隔
		public float multipleTime = 6.5f;
		//时间倍数(计算五组总共的时间)
		...
		//胜负记录
		public GameObject conCealTween;
		//隐藏打开界面
		...
		int m_lChipNum = 1;
		int[] m_lAllChips = new int[13];
		//筹码种类
		int[] m_lGameChips = new int[7];
		//显示筹码
		long[] m_lChipScore = new long[7];
		//筹码金额
		long m_lLastJetton = 0;
		//上一局所选筹码值
		...
		//下庄按钮
		public GameObject o_player_info;
		//当前玩家的信息界面
		public GameObject o_banker_info;
		//庄家的信息界面
		...
		//提示框
		public GameObject o_result;
		//结算界面
		public GameObject o_close_box;
		//退出按钮

		private GameObject o_chip_buttons = null;
		//金币按钮父级
		private GameObject o_bank_player = null;
		//庄家信息
		private GameObject o_add_buttons = null;
		//加钱按钮
		private GameObject o_gameTime_label = null;
		//游戏状态提示
		private GameObject o_clock = null;
		//时钟
		private GameObject o_clock_num = null;
		//时钟数值

		//显示筹码
		private GameObject ctr_chip_tian = null;
		//天
		private GameObject ctr_chip_di = null;
		//地
		private GameObject ctr_chip_xuan = null;
		//玄
		private GameObject ctr_chip_huang = null;
		//黄
		
		private byte[][] m_cbTableCardArray = new byte[GameLogic.AREA_COUNT][];
		//桌面扑克
		
		private byte[] m_cbTableCardArrayAll = new byte[25];
		//桌面扑克

		public  byte[] m_cbCardType = new byte[GameLogic.AREA_COUNT];
		//牌型信息

		public static long[] s_lTableScore = new long[GameLogic.AREA_COUNT];
		//当前玩家在各区域下注筹码(取值)
		public static GameObject[] o_player_chips = new GameObject[GameLogic.AREA_COUNT];
		//当前玩家在各区域下注筹码(UI显示)
		private long[] m_lAreaInAllScore = new long[GameLogic.AREA_COUNT];
		//每个区域下注总分(取值)
		private GameObject[] o_chips_count = new GameObject[GameLogic.AREA_COUNT];
		//每个区域下注总分（UI显示）
		...
		//通用数据
		private static bool s_bReqQuit = false;
		//玩家是否离开
		private static bool s_bStart = false;
		//游戏是否开始
		private static TimerType s_bTimerType = TimerType.TIMER_NULL;
		//游戏状态（空闲、下注、开牌）
		private static float s_nQuitDelay = 0;
		//记录游戏开始到退出游戏的时间（毫秒）
		private static float s_nInfoTickCount = 0;
		//记录系统启动后的时间（接受Environment.TickCount）
		public  static byte s_bBankerUser = GameLogic.NULL_CHAIR;
		//作庄玩家
		private static int s_lGameCount = 0;
		//玩家游戏局数

		private long player_totalMoney = 0;
		//玩家总金币
		private long banker_totalMoney = 0;
		//庄家总金币
		private byte[] m_buffer = new byte[100];
		//记录走势
		private GameObject m_MySelfScorelbl;
		//玩家金币（输赢金币显示）
		private GameObject m_BanckScorelbl;
		//庄家金币（输赢金币显示）
		private long m_selfScore;
		//玩家金币（输赢金币）
		private long m_bankerScore;
		//庄家金币（输赢金币）

		//游戏变量
		private long m_lMaxChipBanker;
		//最大下注 (庄家)
		private byte m_cbTimeLeave;
		//剩余时间
		private int m_nChipTime;
		//下注次数 (本局)
		private int m_nChipTimeCount;
		//已下次数 (本局)
		private long m_nListUserCount;
		//列表人数 (申请上庄)
		private long m_AreaLimit;
		//区域限制
		private bool m_canChip;
		//是否可下注

		private Vector3 m_vecClick = new Vector3 (0, 0, 0);
		//筹码位置
		private List<PlayerInfo> bankerList = new List<PlayerInfo> ();
		//申请庄家列表
		private byte[][] result_notes = new byte[GameLogic.AREA_COUNT][];
		//记录走势
		private byte notesData_leng = 0;
		//记录信息长度

		private long m_lCurrentJetton;
		//当前筹码(当前玩家选择的筹码)
		private bool m_bShowChangeBanker;
		//轮换庄家
		private bool m_bNeedSetGameRecord;
		//完成设置
		private long m_ApplyCondition;
		//上庄条件

		//庄家信息
		private ushort m_wCurrentBanker;
		//当前庄家
		private ushort m_wBankerUser;
		//实际庄家
		private ushort m_wBankerTime;
		//做庄次数
		private long m_lBankerScore;
		//庄家积分（空值）
		private long m_lBankerWinScore;
		//庄家总金币
		private bool m_bEnableSysBanker;
		//是否系统做庄
		private bool applyBanker = false;
		//玩家是否申请上庄
		//private long   m_lTmpBankerWinScore;												//庄家本轮成绩

		private int startBankerTime;
		//普通坐庄次数
		private int allBankerTime;
		//可做庄次数

		//玩家成绩
		private long m_lMeCurGameScore;
		//我的成绩
		private long m_lMeCurWinScore;
		//本轮成绩
		private long m_lMeCurGameReturnScore;
		//本次返回金币
		private long m_lMeGameScoreCount;
		//玩家总成绩
		private long m_lGameRevenue;
		//游戏税收
		private long m_lMeStatisticScore;
		//游戏成绩

		//游戏记录
		private GameObject o_notes;
		//游戏记录界面
		private GameObject[] notes_data = new GameObject[5];
		//显示输赢
		...
		void SetChipScore ()
		{
			...
			for (int i = 0; i < 6; i++) {
				...
				case 8:
					...
					break;
				case 9:
					m_lChipScore [i] = 10000;
					break;
				case 10:
					m_lChipScore [i] = 50000;
					break;
				case 11:
					m_lChipScore [i] = 100000;
					break;
				case 12:
					m_lChipScore [i] = 500000;
					break;
				case 13:
					m_lChipScore [i] = 1000000;
					break;
				}
			}
			SetAddBtn ();
		...
		{
			if (UIManager.Instance.SceneType != enSceneType.SCENE_GAME)
				return;
...
//             }
		...
		void SwitchPlaySceneView (NPacket packet)
		{
			...
			if (isMid == true) {
				Debug.LogWarning ("3333333333中途进来的游戏数据" + m_cbTableCardArrayAll.Length + "第一个" + m_cbTableCardArrayAll [0] + "第二个" + m_cbTableCardArrayAll [1]);
				isFirst = true;
				byte cardCount = 25;
				controlCard.GetComponent<UICardControl> ().SetTwoCardData (m_cbTableCardArrayAll, cardCount);
				//显示先后调整
				Invoke ("FirstSendEffectView", 0.1f);
...
//             }
		...
		{ 
			...
			case SubCmd.SUB_S_DISPATCH_CARD:
				{
					//先发两张牌
					Debug.LogWarning ("先发两张牌");
					OnLicensing (packet);
//                        OnSubUserCancelBanker(packet);
					break;
				}
...
//			controlCard.GetComponent<UICardControl> ().SetCardData (m_cbTableCardArrayAll, (byte)m_cbTableCardArrayAll.Length);
			controlCard.GetComponent<UICardControl> ().ClearCard ();
		...
		//第一次发牌
		void OnLicensing (NPacket packet)
		{
			packet.BeginRead ();
			//扑克信息
			int i = 0;
...
//			for (int idCount = 0; idCount < m_cbCardType.Length; idCount++) {
//				m_cbCardType [idCount] = packet.GetByte ();
//				Debug.LogWarning ("牌型1：" + m_cbCardType [idCount]);
//			}
...
//			UpdateButtonContron();
		...
		//第一次发牌效果
		void FirstSendEffectView ()
		{
			controlCard.GetComponent<UICardControl> ().FirstSendOutTween (sendCardTypeTime, false, isFirst);
		}
		...
		void SendEffectView ()
		{
			controlCard.GetComponent<UICardControl> ().SendOutTween (sendCardTypeTime, false, isFirst);
		}

		//开牌效果
		void ShowEffectView ()
		{
			controlCard.GetComponent<UICardControl> ().CardRotationTween (showCardTypeTime, true, isFirst);
		}
