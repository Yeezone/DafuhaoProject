...
namespace com.QH.QPGame.BRPM
{
	...
	public class label_number : MonoBehaviour
	{
		/// <summary>
        /// 万字，亿字等显示
        /// </summary>
		...
		void Update()
		{
            ...
            if (m_bIsTimer && m_bIsOpen && m_fWorkTime >= m_fSpeed && m_iNum >= 0)
            {
                ...
                {                    
                    ...
                    AudioSoundManger_PM._instance.StopTimerSound();
                    AudioSoundManger_PM._instance.PlaySound("clock");
                }
                if (m_iNum == 0)
                {
                    AudioSoundManger_PM._instance.StopSound("clock");
                    m_bIsOpen = false;
                }
                m_fWorkTime = 0;
			}
		...
		private void SetNum()
		{
            ...
            if (m_bIsSign && _iNum != 0)
			{
                ...
                temp_gobj.spriteName = m_strTextureName + strname;
				temp_gobj.transform.name = strname;
				temp_gobj.GetComponent<UISprite>().color = _cColor;
				temp_gobj.GetComponent<UIWidget>().width = _iPerNumWidth;
				temp_gobj.GetComponent<UIWidget>().height = _iPerNumHeight;
